...
def test_migrations_apply_and_seed():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        completed = subprocess.run([sys.executable, script, '--db', db_path, '--migrations', migrations_dir], capture_output=True, text=True)
        assert completed.returncode == 0, f"Migration runner failed:\nSTDOUT:\n{completed.stdout}\nSTDERR:\n{completed.stderr}"

        conn = sqlite3.connect(db_path)
        cur = conn.cursor()

        # Expected tables
        expected_tables = ('campaigns', 'sessions', 'runtime_config', 'schema_migrations', 'command_receipts')
        ...
        assert any(v.startswith('0001') for v in versions), '0001 migration not recorded in schema_migrations'
        assert any(v.startswith('0002') for v in versions), '0002 migration not recorded in schema_migrations'
